...
def run_round(monkeys, big_product):
    for monkey in monkeys:
        throws = monkey.inspect_items()
        for throw in throws:
            monkeys[throw].items.append(monkey.items.pop(0) % big_product)
...
def main(lines, num_rounds=20, worry_level_management=3):
    ...
    big_product = math.prod(all_divisors)
    for round_num in range(num_rounds):
        if round_num % 100 == 0:
            print(f'{round_num=}')
        run_round(monkeys, big_product)
