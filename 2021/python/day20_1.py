def extend_image(input_image):
    extended_image = []
    extended_image.append('.' * (len(input_image[0]) + 4))
    extended_image.append('.' * (len(input_image[0]) + 4))
...
def get_binary_representation(y, x, extended_image):
    square_rep = []
...
def process_image(image, algorithm):
    ...
    for y, line in enumerate(image):
        ...
        for x, c in enumerate(line):
            index = get_binary_representation(y, x, image)
            print(index)
...
if __name__ == '__main__':  # 5908 is too high
