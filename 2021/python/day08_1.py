...
def main(lines):
    ...
    # num_4s = len([line for line in lines if output_contains_how_many(line, 4)])
    # num_7s = len([line for line in lines if output_contains_how_many(line, 7)])
    # num_8s = len([line for line in lines if output_contains_how_many(line, 8)])
    print(f'{num_1s=}')
    print(f'{num_4s=}')
    print(f'{num_7s=}')
    print(f'{num_8s=}')
